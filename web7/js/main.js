...
// ===================================
// АДАПТИВНЫЙ ПОДСТРОЧНИК С ФИКСИРОВАННОЙ ВЫСОТОЙ
// ===================================

window.TransLamps.utils.adjustBrandSubtitle = function() {
    const subtitles = document.querySelectorAll('.brand-subtitle, .brand .small, .brand-link .text-silver, .brand-link .small');
    ...
    subtitles.forEach(subtitle => {
        ...
        subtitle.style.webkitLineClamp = '';
        
        // Определяем базовый размер в зависимости от длины текста
        if (textLength <= 20) {
            subtitle.setAttribute('data-length', 'short');
            subtitle.style.fontSize = '18px';
        } else if (textLength <= 40) {
            subtitle.setAttribute('data-length', 'medium');
            subtitle.style.fontSize = '16px';
        } else {
            subtitle.setAttribute('data-length', 'long');
        ...
        // Если текст не помещается по ширине, уменьшаем шрифт
        if (textWidth > parentWidth) {
            const currentSize = parseInt(window.getComputedStyle(subtitle).fontSize);
            
            // Пробуем уменьшить размер шрифта
            if (currentSize > 12) {
                const newSize = Math.max(12, currentSize - 2);
                subtitle.style.fontSize = newSize + 'px';
                subtitle.style.lineHeight = newSize + 'px';
            }
            
            // Если всё равно не помещается, разрешаем перенос
            subtitle.style.whiteSpace = 'nowrap';
            const newTextWidth = subtitle.scrollWidth;
            
            if (newTextWidth > parentWidth) {
...
// Инициализация
document.addEventListener('DOMContentLoaded', function() {
    window.TransLamps.utils.adjustBrandSubtitle();
    
    // Обновляем при изменении размера окна
    let resizeTimer;
    window.addEventListener('resize', function() {
        clearTimeout(resizeTimer);
        resizeTimer = setTimeout(window.TransLamps.utils.adjustBrandSubtitle, 250);
    });
});
